...
function move() {
   ...
   if (snakeBody[0].getAttribute(`posX`) == apple.getAttribute(`posX`) && snakeBody[0].getAttribute(`posY`) == apple.getAttribute(`posY`)) {
        ...
        let b = snakeBody[snakeBody.length-1].getAttribute(`posX`);
   ...
   if (snakeBody[0].classList.contains(`snakeBody`)) {
    ...
    snakeBody[0].style.background = `url(саt.jpg) center no-repeat`;
...
if (snakeBody>localStorage.getItem(`top20`)) {
    record=snakeBody;
    localStorage.setItem(`top20`, record);
};
